...
import { findActiveTab, getTabKey, useTabStore, type Tab } from '../../stores/tabStore'
...
export function ContentRouter() {
  ...
  const recentSessionKeys = useTabStore((s) => s.recentSessionKeys)
  const activeTab = findActiveTab(tabs, activeTabKey, activeTabId)
  const activeTabType = activeTab?.type
  const terminalTabs = tabs.filter((tab) => tab.type === 'terminal')
  const activeSessionKey = activeTab && activeTabType === 'session' ? getTabKey(activeTab) : null
  const recentKeys = recentSessionKeys.length > 0
    ? recentSessionKeys
    : recentSessionIds
        .map((sessionId) => tabs.find((tab) => tab.sessionId === sessionId && tab.type === 'session'))
        .filter((tab): tab is Tab => Boolean(tab))
        .map(getTabKey)
  const sessionPanelKeys = [
  ...
  const showEmptySession = !activeTab || !activeTabType
  ...
  return (
    <div className="relative min-h-0 flex-1 overflow-hidden">
      ...
      {sessionPanelKeys.map((panelKey) => {
        const tab = tabs.find((candidate) => getTabKey(candidate) === panelKey)
        if (!tab) return null
        const isActive = panelKey === activeSessionKey && activeTabType === 'session'
        return (
          <div
            key={panelKey}
            aria-hidden={!isActive}
            data-session-panel={panelKey}
          ...
          >
            <ActiveSession sessionId={tab.sessionId} projectPath={tab.projectPath} isActive={isActive} />
          </div>
        )
      })}
      ...
      {terminalTabs.map((tab) => {
        const active = activeTab ? getTabKey(tab) === getTabKey(activeTab) : false
