...
import powSolverPath from "./deepseek-pow-solver.cjs" with { type: "file" };
import sha3WasmPath from "./sha3_wasm_bg.wasm" with { type: "file" };
...
async function getWasmSolver(): Promise<DeepSeekHashWasm | null> {
  ...
  try {
    ...
    await solver.init(sha3WasmPath);
    _wasmSolver = solver;
    return solver;
  } catch {
