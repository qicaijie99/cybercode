...
桌面端不仅提供本地 GUI，还支持通过 **微信、QQ、Telegram 和飞书** 远程对话。
...
2. 填入平台凭证：
   - **微信**：使用手机微信扫码
   - **QQ**：扫码绑定，或 AppID + App Secret
   - **Telegram**：Bot Token
   - **飞书**：App ID + App Secret
3. 配置默认工作目录和允许的用户
4. 保存后桌面端会自动启动或重启适配器进程
...
3. 在 QQ/Telegram/飞书中向 Bot 发送该配对码（微信和 QQ 扫码用户会自动配对）
4. 配对成功后即可开始对话
...
权限请求会尽可能以按钮形式展示；所有平台也都可使用 `/allow` 和 `/deny` 回复审批。
